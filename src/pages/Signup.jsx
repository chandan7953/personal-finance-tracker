...
const SignUpSignIn = () => {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      // Check if the user exists before creating the document
      if (user) {
        await createUserDocument(user);
        toast.success("User Authenticated Successfully!");
        navigate("/dashboard");
      } else {
        throw new Error("User authentication failed."); // Throw an error if user is null
      }
    } catch (error) {
      toast.error(error.message);
      console.error("Error signing in with Google: ", error.message);
    } finally {
      setLoading(false); // Always reset loading state after the operation completes
    }
  };
